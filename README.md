...
A refusal is a **loud skip, not an abort** (DF-TERMINAL-JAIL-21): the validator refuses only the pack itself — one reason line on stderr, and **nothing written for that pack** — while the base install (wrapper, lib tree, default rules file) always completes. The installer prints a final summary naming the installed wrapper and every skipped pack, and exits `2` so install automation notices. An unknown pack name is the same kind of skip and suggests `--list-rule-packs`. The listing itself is informational and never refuses: a pack it cannot parse (for example on a host without PyYAML) is reported as `name: unreadable (reason)` and the listing still exits `0`, so `install.sh --list-rule-packs && install.sh` bootstrap chains keep working.
